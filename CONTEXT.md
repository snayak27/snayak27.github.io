...
- `computer-science/timeline.html` causal-graph timeline: refactored centered modal to landscape layout, placing the visual mind map (left) and floor cascade stack (right) side-by-side. Reduces modal height to prevent cropping, ensuring full layout visibility. Stacks responsive columns on screen widths under 820px.
