...
(function () {
  ...
  function buildThemeBtn() {
    ...
    btn.addEventListener('click', function () {
      ...
      applyTheme(next);
      try { localStorage.setItem('sap-theme', next); } catch (e) {}
      paint();
    });
    return btn;
  }
  ...
  function buildOverlay() {
    ...
    ov.setAttribute('aria-label', 'Search the SAP Landscape guide');
    ov.innerHTML =
      '<div class="search-panel">' +
        '<input class="search-input" type="text" placeholder="Search every page — products, roles, processes…" aria-label="Search">' +
  ...
  function render(q) {
    ...
    if (!SEARCH.hits.length) {
      html = '<div class="search-empty">No pages match “' + esc(q) + '”. Try “BTP”, “CFO”, or “cloud”.</div>';
    } else {
